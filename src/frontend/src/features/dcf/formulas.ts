...
export function calculateAllFormulas(inputs: DcfInputs): FormulasRegistry {
  ...
  const bookValueValue =
    safeDivide(safeNum(ltp), safeNum(pbRatio)) * totalSharesValue;
  ...
  const initialGrowthValue =
    safeNum(revenueLastQuarter) * 4 - safeNum(revenueLastYear);
  ...
  const CValue =
    safeDivide(safeNum(netProfitLastYear), safeNum(marketCap)) * 100;
  ...
  const EValue =
    safeDivide(safeNum(revenueLastQuarter), safeNum(marketCap)) * 4;
  ...
  const FValue =
    safeDivide(safeNum(netProfitLastQuarter), safeNum(marketCap)) * 400;
  ...
  const initialAverageCapitalReturnValue =
    (safeNum(initialCapitalReturnYearlyValue) +
      safeNum(initialCapitalReturnQuarterlyValue)) /
    2;
  const initialAverageCapitalReturn: FormulaDefinition = {
    name: "Initial Average Capital Return",
    expression:
      "(Initial Capital Return Yearly + Initial Capital Return Quarterly) ÷ 2",
